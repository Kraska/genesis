...
export class ProgressStorage {
  ...
  constructor(
    ...
    this.progressMap = items.reduce<Record<string, Progress>>(
      (map, { id, duration }) => {
  ...
  private load(): void {
    ...
    Object.entries(this.timesMap).forEach(([id, time]) =>
      this.updateItemProgress(id, time)
    );
  }
  ...
  public getProgress(): Progress {
    const progress = Object.values(this.progressMap).reduce(
  ...
  public getCurrentItem(excludedIds: string[] = []): ProgressItem | null {
    return (
      this.items.find(
        ({ id }) =>
          !excludedIds.includes(id) && this.progressMap[id].persent < 100
      ) || null
    );
  }
}
