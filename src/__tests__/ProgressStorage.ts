...
const items: ProgressItem[] = [lesson1, lesson2, lesson3];
...
describe("Home Page", () => {
  ...
  test("When LS is empty, return zero-progress", () => {
    ...
    expect(ps.getItemProgress("lesson1")).toEqual({
      time: 0,
      persent: 0,
      duration: 100,
    });
  ...
  test("Calculating common progress", () => {
    ...
    expect(ps.getProgress()).toEqual({ time: 150, persent: 25, duration: 600 });
  });
